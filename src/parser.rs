...
lazy_static! {
    static ref PRECCLIMBER: PrecClimber<Rule> = PrecClimber::new(vec![
...
impl PrometheusParser {
    ...
    fn selector(input: Node) -> Result<Expression> {
        ...
        for child in input.into_children() {
            match child.as_rule() {
                ...
                r => return Err(child.error(format!("invalid rule {:?} in selector", r))),
            }
        }
    ...
    fn function(input: Node) -> Result<Expression> {
        ...
        for child in input.children() {
            match child.as_rule() {
                ...
                r => return Err(child.error(format!("invalid rule {:?} in function", r))),
            }
        }
    ...
    fn expression(lhs: Expression, op: Node, rhs: Expression) -> Result<Expression> {
        ...
        let kind = match op.as_rule() {
            ...
            op_greater_than_bool => OperatorKind::GreaterThanBool,
            op_greater_than_equal => OperatorKind::GreaterThanEqual,
            op_greater_than_equal_bool => OperatorKind::GreaterThanEqualBool,
            op_less_than => OperatorKind::LessThan,
            op_less_than_bool => OperatorKind::LessThanBool,
            op_less_than_equal => OperatorKind::LessThanEqual,
            op_less_than_equal_bool => OperatorKind::LessThanEqualBool,
            op_not_equal => OperatorKind::NotEqual,
            op_not_equal_bool => OperatorKind::NotEqualBool,
            op_equal => OperatorKind::Equal,
            op_equal_bool => OperatorKind::EqualBool,
